    ...
    <>
      ...
      <main id="main" className="min-h-screen bg-cream">
        <div className="mx-auto max-w-[1100px] px-6 py-10">
          ...
          <div className="mt-8">
            <TeamNav
              current="/team/applications"
              counts={{ "/team/applications": waiting }}
            />
          </div>
          ...
          <form className="mt-8 flex flex-wrap items-center gap-3" action="/team/applications">
            ...
            {q ? (
              <Link
                href={`/team/applications?show=${show.key}`}
          ...
          <nav className="mt-6 flex flex-wrap gap-2" aria-label="Filter">
            {FILTERS.map((f) => (
              <Link
                key={f.key}
                href={`/team/applications?show=${f.key}${q ? `&q=${encodeURIComponent(q)}` : ""}`}
